...
def run_for_patch(well):
    assert well.shape == (1, 1, 3, 224, 224), f"There should be a single well in the input, it was {well.shape}"
    with torch.no_grad():
        this_well = percentile_normalize(torch.Tensor(well[0, 0]))
        out = infer(model, this_well[None, ...])["quantized_embed"].squeeze(-1).squeeze(-1).numpy()
    return out[None, ...]
